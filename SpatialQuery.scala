...
object SpatialQuery extends App{
  def ST_Contains(queryRectangle:String,pointString:String): Boolean ={
    ...
    val xy = pointString.split(",").map(_.toDouble)
    if(rectangle(0) > xy(0) | rectangle(2) < xy(0) | rectangle(1) > xy(1) | rectangle(3) < xy(1)) {
      return false
    }
    return true
  }
  ...
  def ST_Within(pointString1:String, pointString2:String, distance:Double): Boolean ={
    ...
    val edist = math.sqrt(math.pow((point1(0) - point2(0)),2) + math.pow((point1(1) - point2(1)), 2))
    if (edist <= distance) {
      return true
    }else {
      return false
